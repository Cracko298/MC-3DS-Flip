...
def extract_colors(image_path):
    ...
    existing_colors = set()  # Keep track of existing colors
    
    # Load existing colors from the output file, if it exists
    try:
...
if __name__ == "__main__":
    ...
    if sys.argv[1+i] == 'help' or sys.argv[2+i] == 'help' or sys.argv[1+i] == '?' or sys.argv[2+i] == '?':
        print("Usage: .\m3dsflip <flag> <command> <image_path>")
        print("Helpful Hint: Simple Commands don't require a Flag.\n")
        sys.exit(1)

    
    flip_direction = sys.argv[1+i].lower()
    ...
    if yflip == True or xflip == True or dstflip == True or convert == True:
        image_path = sys.argv[2+i]
    
    if filter_contrast == True:
        image_path = sys.argv[3+i]
        color = sys.argv[2]
