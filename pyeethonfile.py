...
class FileSelectorApp:
    ...
    def create_widgets(self):
        ...
        files.insert(0, "Select File")  # Add "Select File" as the first option
        file_var = tk.StringVar()
        file_var.set(files[0])  # Set the default file in the dropdown

        # Configure the dropdown menu for an outlined appearance
        style = ttk.Style()
        ...
        dropdown.place(x=10, y=10)  # Adjust the position of the dropdown

        # Create a button to perform some action with the selected file path (you can replace this with your logic)
        action_button = ttk.Button(self.root, text="Extract Colors", command=self.color_extract)
        action_button.place(x=10, y=50)  # Adjust the position of the button

        # Create settings button with gear Unicode character
    ...
    def get_files_with_extensions(self, *extensions):
        ...
        if os.path.exists("mc3dsflip.config"):
            ...
            if fourth_line == "True":
                files = []
                for foldername, subfolders, filenames in os.walk('.'):  # Change '.' to the desired root directory
    ...
    def update_selected_file(self, selected_file):
        # Update the selected file path variable, excluding the "Select File" option
    ...
    def color_extract(self):
        image_path = self.selected_file_path.get() # Gets Selected File
